Public Class frm_CheckbookDepositsATM
    ...
    Dim booCorrection, booDataChecked, booListbox, booListboxOpen As Boolean
    Dim strInitEntry, sendername As String
    Dim CategoryArray(1) As Integer
    ...
    Private Sub frm_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        strSQL = "SELECT CompanyName,CompanyAddress,CompanyCity,CompanyState,CompanyZipCode,CompanyPhone FROM System;"
        ...
        Else
            ...
            If dbDataSet_Misc.Tables("Category").Rows.Count <> 0 Then 'Records found  
                ...
                Next
            Else
                frm_Message.Text = "16:1:0:1:3:Tax categorys must be set up before ATM transactions can be processed"
    ...
    Private Sub Save()
        ...
        'Retreive tax category data
        strSQL = "SELECT Category FROM TaxCategorys ORDER BY Id;"
        dbDataSet_Misc.Clear()
        dbDataSet_Misc.Tables.Clear()
        gbl_DstConnect.Open()
        dbAdapter_Misc = New OleDb.OleDbDataAdapter(strSQL, gbl_DstConnect)
        dbAdapter_Misc.Fill(dbDataSet_Misc, "TaxCategories")
        gbl_DstConnect.Close()
        Dim TaxCatArray(dbDataSet_Misc.Tables("TaxCategories").Rows.Count - 1)
        For i = 0 To 17
            TaxCatArray(i) = dbDataSet_Misc.Tables("TaxCategories").Rows(i).Item(0) 'Tax category
        Next

        'Retreive banking data
        strSQL = "Select Id,TransDate,Transactions,Payee,Category,Memos,Amount,Balance,Clear FROM Banking ORDER BY Id;"
        dbDataSet.Clear()
        dbDataSet.Tables.Clear()
        gbl_DstConnect.Open()
        ...
        'Retreive balance 
        Dim TransAmount, BankBalance As Double 'As Single
        BankBalance = dbDataSet.Tables("Transaction").Rows(dbDataSet.Tables("Transaction").Rows.Count - 1).Item(7)

        For intStep = 0 To LineCount 'Save deposit/atm data to database
            If IsNumeric(tbox_Amount(intStep).Text) Then 'Test for numeric entry
                If tbox_Amount(intStep).Text > 0 Then 'Save only if positive number in amount 
                    ...
                    dsNewRow = dbDataSet.Tables("Transaction").NewRow()
                    If tbox_Date.Text = "  /  /" Then dsNewRow.Item("TransDate") = DBNull.Value Else dsNewRow.Item("TransDate") = tbox_Date.Text
                    TransAmount = Convert.ToDouble(Replace(tbox_Amount(intStep).Text, "$", "")) 'Convert.ToSingle
                    If Me.Text = "Deposits" Then
                    ...
                    dbDataSet.Tables("Transaction").Rows.Add(dsNewRow)
                    dbAdapter.Update(dbDataSet, "Transaction")
                    cb0 = Nothing

                    If Me.Text = "ATM Withdrawls" Then 'Update checkpayee list on atm transactions

                        'Retreive index for selected category
                        Dim CatIndex As Integer
                        For CatIndex = 0 To 17
                            If TaxCatArray(CatIndex) = Trim(tbox_Category(intStep).Text) Then Exit For
                        Next

                        strSQL = "SELECT Id,Payto,Category FROM CheckPayee WHERE Payto = '" & Trim(tbox_TransactionName(intStep).Text) & "';"
                        dbDataSet_Misc.Clear()
                        dbDataSet_Misc.Tables.Clear()
                        gbl_DstConnect.Open()
                        dbAdapter_Misc = New OleDb.OleDbDataAdapter(strSQL, gbl_DstConnect)
                        dbAdapter_Misc.Fill(dbDataSet_Misc, "PayeeUpdate")
                        gbl_DstConnect.Close()

                        Dim cb1 As New OleDb.OleDbCommandBuilder(dbAdapter_Misc)
                        If dbDataSet_Misc.Tables("PayeeUpdate").Rows.Count <> 0 Then 'Update existing checkpayee listing
                            dbDataSet_Misc.Tables("PayeeUpdate").Rows(0).Item(1) = StrConv(Trim(tbox_TransactionName(intStep).Text), VbStrConv.ProperCase)
                            dbDataSet_Misc.Tables("PayeeUpdate").Rows(0).Item(2) = CatIndex
                        Else 'Add new checkpayee entry
                            dsNewRow = dbDataSet_Misc.Tables("PayeeUpdate").NewRow()
                            dsNewRow.Item("Payto") = StrConv(Trim(tbox_TransactionName(intStep).Text), VbStrConv.ProperCase)
                            dsNewRow.Item("Category") = CatIndex
                            dbDataSet_Misc.Tables("PayeeUpdate").Rows.Add(dsNewRow)
                        End If
                        dbAdapter_Misc.Update(dbDataSet_Misc, "PayeeUpdate")
                        cb1 = Nothing
                    End If
                    ...
                    Else
                        dt.Rows.Add(tbox_Date.Text, "ATM", Nothing, Trim(tbox_TransactionName(intStep).Text), Trim(tbox_Category(intStep).Text), Trim(tbox_Memo(intStep).Text), TransAmount, BankBalance, False)
                    End If
    ...
    Private Sub cmd_Submit_Click(sender As Object, e As EventArgs) Handles cmd_Submit.Click
        ...
        Select Case intAlarm
            Case 0 'Complete data, save 
                ...
                Else
                    If Me.Text = "Deposits" Then
                        frm_Message.Text = "16:1:0:1:3:Depositors name with amount must be entered to continue"
                    Else
                        frm_Message.Text = "16:1:0:1:3:Creditors name with amount must be entered to continue"
                    End If
                End If
            Case 1 'Missing transaction name
                If Me.Text = "Deposits" Then
                    frm_Message.Text = "16:1:0:1:3:Missing depositor name with value entered into amount"
                Else
                    frm_Message.Text = "16:1:0:1:3:Missing creditor name with value entered into amount"
                End If
            Case 2 'Missing amount
                If Me.Text = "Deposits" Then
                    frm_Message.Text = "16:1:0:1:3:Missing amount associated with entered depositor name"
                Else
                    frm_Message.Text = "16:1:0:1:3:Missing amount associated with entered creditor name"
                End If
        End Select
        frm_Message.ShowDialog()
    End Sub
    Private Sub ListBox_Names_DoubleClick(sender As Object, e As EventArgs) Handles ListBox_Names.DoubleClick
        If Me.Text = "ATM Withdrawls" Then 'Pre select category if possible
            If CategoryArray(ListBox_Names.SelectedIndex) <> -1 Then
                opt_Category(CategoryArray(ListBox_Names.SelectedIndex)).Checked = True
            Else
                For i = 0 To 17
                    opt_Category(i).Checked = False
                Next
        ...
        booDataChecked = False
        If Me.Text = "ATM Withdrawls" Then
            If tbox_Category(ActiveRow).Text = "" Then tbox_Category(ActiveRow).Focus() Else tbox_Amount(ActiveRow).Focus()
        Else : tbox_Amount(ActiveRow).Focus() : End If
    End Sub
    Private Sub ListBox_Names_VisibleChanged(sender As Object, e As EventArgs) Handles ListBox_Names.VisibleChanged
        If ListBox_Names.Items.Count <> 0 Then
            If booListboxOpen = False Then booListboxOpen = True : Exit Sub
            FormEnable(True)
            booListboxOpen = False
        End If
    End Sub
        ...
        Select Case sender.name
            Case "tbox_Date"
                If IsDate(sender.Text) = False And sender.Text <> "  /  /" Then
                    ...
                    booCorrection = True
                    frm_Message.Text = "16:1:0:1:3:The date entered is not valid, Please enter a valid date"
                    frm_Message.ShowDialog()
                    sender.text = ""
                    sender.Focus()
                    booCorrection = False
            ...
            Case "tbox_Amount"
                If sender.text <> "" Then
                    If Not IsNumeric(sender.Text) Then
                        booCorrection = True
                        frm_Message.Text = "16:1:0:1:3:Invalid Entry --- Numerical entries are only allowed"
                        frm_Message.ShowDialog()
                        sender.text = ""
                        sender.Focus()
                        booCorrection = False
    ...
    Private Sub tbox_TextChanged(sender As Object, e As EventArgs) Handles tbox_Date.TextChanged
        ...
        Select Case sender.name
            ...
            Case "tbox_TransactionName"
                ...
                Else
                    strSQL = "SELECT Payto,Category FROM CheckPayee WHERE Payto LIKE '" & Trim(tbox_TransactionName(sender.tag).Text) & "%" & "' ORDER BY Payto;"
                End If
                dbDataSet_Misc.Clear()
                dbDataSet_Misc.Tables.Clear()
                gbl_DstConnect.Open()
                ...
                If dbDataSet_Misc.Tables("ListboxNames").Rows.Count >= 1 Then 'Records found 
                    If Me.Text = "ATM Withdrawls" Then ReDim CategoryArray(dbDataSet_Misc.Tables("ListboxNames").Rows.Count - 1) 'Set up array to hold selected categorys for listbox list
                    ...
                    For num = 0 To dbDataSet_Misc.Tables("ListboxNames").Rows.Count - 1
                        ...
                        If Me.Text = "ATM Withdrawls" Then 'Fill array with selected catagory 
                            If Not IsDBNull(dbDataSet_Misc.Tables("ListboxNames").Rows(num).Item(1)) Then
                                CategoryArray(num) = dbDataSet_Misc.Tables("ListboxNames").Rows(num).Item(1)
                            Else : CategoryArray(num) = -1 : End If
                        End If
                    Next
    ...
    Private Sub tbox_KeyPress(sender As Object, e As KeyPressEventArgs) Handles tbox_Date.KeyPress
        ...
        If e.KeyChar = ChrW(Keys.Return) Then
            Select Case sender.Name
                ...
                Case "tbox_TransactionName"
                    If Me.Text = "Deposits" Then tbox_Amount(sender.tag).Focus() Else tbox_Category(sender.tag).Focus()
                ...
                Case "tbox_Amount"
                    If sender.tag < LineCount And sender.text <> "" Then
                        ...
                        If Me.Text = "ATM Withdrawls" Then Call OptionUpdate()
